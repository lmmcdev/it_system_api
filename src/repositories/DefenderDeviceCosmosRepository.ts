/**
 * CosmosDB repository for Defender Device data access (write operations)
 * Handles device storage, UPSERT operations, and sync metadata tracking
 *
 * Security Features:
 * - Parameterized queries to prevent injection
 * - 429 throttling with exponential backoff (IMPROVED RETRY LOGIC)
 * - Request timeouts
...
export class DefenderDeviceCosmosRepository {
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    try {
      ...
      this.devicesContainer = this.database.container(config.defenderDeviceSync.containerId);
      this.isInitialized = true;

      logger.info('[CosmosDB] Defender Device Cosmos Repository initialized successfully', {
        database: config.cosmos.databaseId,
        container: config.defenderDeviceSync.containerId
      });
    } catch (error) {
      logger.error('[CosmosDB] Failed to initialize Defender Device Cosmos Repository', error as Error);
  ...
  /**
   * Exponential backoff retry for throttling (IMPROVED RETRY LOGIC)
   */
  ...
  /**
   * Upsert a single defender device
   * Uses UPSERT to insert new or update existing device
   */
  async upsertDevice(device: DefenderDevice): Promise<DefenderDevice> {
    const startTime = Date.now();

    try {
      const container = await this.getDevicesContainer();

      logger.info('[CosmosDB] Upserting defender device', {
        id: device.id,
      ...
      logger.info('[CosmosDB] Defender device upserted successfully', {
        id: device.id,
        computerDnsName: device.computerDnsName,
      ...
      return resource as DefenderDevice;
    } catch (error) {
      const executionTime = Date.now() - startTime;
      ...
      logger.error('[CosmosDB] Failed to upsert defender device', error as Error, {
        id: device.id,
        computerDnsName: device.computerDnsName,
      ...
      throw new ServiceError('Failed to upsert defender device');
    }
  }

  /**
   * Bulk upsert defender devices using CosmosDB bulk operations
   * Processes devices in batches for optimal performance and RU consumption
   * INCLUDES IMPROVED THROTTLE RETRY LOGIC
   *
   * @param devices - Array of devices to upsert
   * @returns Result with success/failure counts and errors
   */
  async bulkUpsertDevices(devices: DefenderDevice[]): Promise<BulkUpsertResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      const operations = devices.map(device => ({
        ...
        resourceBody: device as any // CosmosDB SDK type issue - DefenderDevice is compatible
      }));
      ...
      const throttledDevices: DefenderDevice[] = [];
      ...
      // IMPROVED RETRY LOGIC: Retry throttled devices individually with exponential backoff
  ...
  async getSyncMetadata(): Promise<SyncMetadata> {
    ...
    try {
      const container = await this.getDevicesContainer();
      const id = 'sync_metadata_defender';
  ...
  async updateSyncMetadata(metadata: SyncMetadata): Promise<SyncMetadata> {
    ...
    try {
      ...
      // Ensure ID is always 'sync_metadata_defender'
      metadata.id = 'sync_metadata_defender';
  ...
  /**
   * Get defender device by ID from CosmosDB
   */
  async getDeviceById(id: string): Promise<DefenderDevice> {
    const startTime = Date.now();

    try {
      const container = await this.getDevicesContainer();

      logger.info('[CosmosDB] Fetching defender device by ID', { id });

      const readPromise = container.item(id, id).read<DefenderDevice>();
      const response = await Promise.race([
        readPromise,
        this.createTimeoutPromise(this.QUERY_TIMEOUT_MS)
      ]);
      ...
      if (!response.resource) {
        logger.warn('[CosmosDB] Defender device not found', { id });
        throw new NotFoundError('Defender Device', id);
      }

      logger.info('[CosmosDB] Defender device fetched successfully', {
        id,
        computerDnsName: response.resource.computerDnsName,
        executionTime: `${executionTime}ms`
      });
      ...
      if ((error as { code?: number }).code === 404) {
        logger.warn('[CosmosDB] Defender device not found (404)', { id, executionTime: `${executionTime}ms` });
        throw new NotFoundError('Defender Device', id);
      }

      logger.error('[CosmosDB] Failed to fetch defender device', error as Error, {
        id,
        errorCode: (error as { code?: number }).code,
        executionTime: `${executionTime}ms`
      });

      throw new ServiceError('Failed to fetch defender device');
    }
  }
...
export const defenderDeviceCosmosRepository = new DefenderDeviceCosmosRepository();
