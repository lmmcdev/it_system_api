/**
 * HTTP GET endpoint for user impact statistics
...
app.http('getUserImpactStatistics', {
  methods: ['GET'],
  authLevel: 'anonymous',
  route: 'statistics/user-impact',
  handler: async (request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> => {
    const functionName = 'getUserImpactStatistics';
    ...
    try {
      ...
      const result = await alertStatisticsService.queryStatistics(
        {
          type: StatisticsType.UserImpact,
      ...
      logger.info('[HTTP] User impact statistics query completed', {
