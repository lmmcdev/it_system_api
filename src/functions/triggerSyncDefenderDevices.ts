/**
 * HTTP trigger to manually initiate defender device synchronization
 * Endpoint: GET /trigger/sync-defender-devices
 *
 ...
 */
async function triggerSyncDefenderDevicesHandler(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  const functionName = 'triggerSyncDefenderDevices';
  ...
  try {
    logger.info('[DefenderDevices Sync] Manual sync triggered via HTTP', {
      functionName,
      invocationId,
    ...
    if (!authResult.authenticated) {
      logger.warn('[DefenderDevices Sync] Authentication failed', {
    ...
    logger.info('[DefenderDevices Sync] Authentication successful', {
    ...
    // Log final results
    if (result.success) {
      logger.info('[DefenderDevices Sync] Manual sync completed successfully', {
      ...
      logger.error('[DefenderDevices Sync] Manual sync completed with errors', undefined, {
        status: result.status,
        devicesProcessed: result.devicesProcessed,
        devicesFailed: result.devicesFailed,
        executionTime: `${executionTime}ms`,
    ...
    logger.error('[DefenderDevices Sync] Manual sync failed with unexpected error', error as Error, {
      functionName,
      invocationId
    });
...
app.http('triggerSyncDefenderDevices', {
  methods: ['GET'],
  authLevel: 'anonymous', // Auth handled in code
  route: 'trigger/sync-defender-devices',
  handler: triggerSyncDefenderDevicesHandler
});
