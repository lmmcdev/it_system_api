/**
 * HTTP GET endpoint for detection source statistics
...
app.http('getDetectionSourceStatistics', {
  methods: ['GET'],
  authLevel: 'anonymous',
  route: 'statistics/detection-sources',
  handler: async (request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> => {
    const functionName = 'getDetectionSourceStatistics';
    ...
    try {
      ...
      const result = await alertStatisticsService.queryStatistics(
        {
          type: StatisticsType.DetectionSource,
      ...
      logger.info('[HTTP] Detection source statistics query completed', {
