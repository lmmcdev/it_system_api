/**
 * HTTP GET endpoint for attack type statistics
...
app.http('getAttackTypeStatistics', {
  methods: ['GET'],
  authLevel: 'anonymous',
  route: 'statistics/attack-types',
  handler: async (request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> => {
    const functionName = 'getAttackTypeStatistics';
    ...
    try {
      ...
      const result = await alertStatisticsService.queryStatistics(
        {
          type: StatisticsType.AttackTypes,
      ...
      logger.info('[HTTP] Attack type statistics query completed', {
