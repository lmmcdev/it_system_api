/**
 * Managed Device Sync Service
 * Orchestrates synchronization of managed devices from Microsoft Graph API to CosmosDB
 *
 * Features:
 * - Fetches all devices from Graph API with pagination
...
import { managedDeviceRepository } from '../repositories/ManagedDeviceRepository';
import { managedDeviceCosmosRepository } from '../repositories/ManagedDeviceCosmosRepository';
...
export class ManagedDeviceSyncService {
  private readonly BATCH_SIZE: number;
  private readonly SYNC_VERSION = '1.0.0';
  private readonly MAX_ERRORS_TO_TRACK = 100; // Limit error array size

  constructor() {
    this.BATCH_SIZE = config.deviceSync.batchSize;
  }

  /**
   * Execute full device synchronization
   * Fetches all devices from Graph API and upserts them to CosmosDB
   *
  ...
  async syncDevices(progressCallback?: SyncProgressCallback): Promise<SyncResult> {
    ...
    try {
      logger.info('[ManagedDevices Sync] Starting device synchronization', {
      ...
      const previousMetadata = await managedDeviceCosmosRepository.getSyncMetadata();
      logger.info('[ManagedDevices Sync] Retrieved previous sync metadata', {
      ...
      logger.info('[ManagedDevices Sync] Phase 1: Fetching devices from Microsoft Graph API');

      let allDevices: ManagedDevice[];
      try {
        allDevices = await managedDeviceRepository.getAllDevicesPaginated();
        graphApiPages = Math.ceil(allDevices.length / 999); // Graph API max page size
        ...
        logger.info('[ManagedDevices Sync] Successfully fetched all devices from Graph API', {
          totalDevices: totalDevicesFetched,
          pages: graphApiPages,
          executionTime: `${totalGraphRequestTimeMs}ms`
        });
      } catch (error) {
        logger.error('[ManagedDevices Sync] Failed to fetch devices from Graph API', error as Error, {
        ...
        return {
          ...
          errors: [{
            deviceId: 'graph_api_fetch',
            error: (error as Error).message || 'Failed to fetch devices from Graph API',
      ...
      if (allDevices.length === 0) {
        logger.warn('[ManagedDevices Sync] No devices returned from Graph API');
        ...
        await managedDeviceCosmosRepository.updateSyncMetadata(metadata);
      ...
      logger.info('[ManagedDevices Sync] Phase 2: Upserting devices to CosmosDB', {
      ...
      logger.info('[ManagedDevices Sync] Created batches for processing', {
        totalBatches,
        batchSize: this.BATCH_SIZE,
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        logger.info('[ManagedDevices Sync] Processing batch', {
          batchNumber,
          totalBatches,
        ...
        try {
          ...
          const result = await managedDeviceCosmosRepository.bulkUpsertDevices(batch);
          ...
          logger.info('[ManagedDevices Sync] Batch processed', {
            batchNumber,
            totalBatches,
          ...
          if (result.failureCount > 0) {
            logger.warn('[ManagedDevices Sync] Some devices failed in batch', {
              batchNumber,
              failureCount: result.failureCount,
          ...
          logger.error('[ManagedDevices Sync] Batch processing failed', error as Error, {
            batchNumber,
            totalBatches,
            batchSize: batch.length
          });
      ...
      logger.info('[ManagedDevices Sync] Synchronization completed', {
        status: syncStatus,
        totalDevicesFetched,
        devicesProcessed,
        devicesFailed,
      ...
      try {
        await managedDeviceCosmosRepository.updateSyncMetadata(metadata);
        logger.info('[ManagedDevices Sync] Sync metadata updated successfully');
      } catch (error) {
        logger.error('[ManagedDevices Sync] Failed to update sync metadata', error as Error);
      ...
      logger.error('[ManagedDevices Sync] Unexpected error during synchronization', error as Error, {
      ...
      try {
        ...
        logger.error('[ManagedDevices Sync] Failed to update metadata after sync error', metadataError as Error);
      }
   ...
   */
  private createBatches(devices: ManagedDevice[], batchSize: number): ManagedDevice[][] {
    const batches: ManagedDevice[][] = [];
  ...
  async getSyncMetadata(): Promise<SyncMetadata> {
    return managedDeviceCosmosRepository.getSyncMetadata();
  }
}

// Export singleton instance
export const managedDeviceSyncService = new ManagedDeviceSyncService();
